...
begin
  ...
  case Str[1] of
    ...
    'B':
      Result := ecCursorDown;
    'C':
      Result := ecCursorRight;
    'D':
      Result := ecCursorLeft;
    'H':
      Result := ecCursorHome;
    'I':
      Result := ecReverseLineFeed;
    'J':
    ...
    '/':
      begin
        ...
        else if (Length(Str) = 2) and (Str = '/Z') then
...
begin
  if Length(Str) = 1 then
    case Str[1] of
      ...
      'c':
        Result := ecReset;
      '7':
        Result := ecSaveCaretAndAttr;
      '8':
        Result := ecRestoreCaretAndAttr;
      '#':
        Result := ecNotCompleted;
      'O':
        Result := ecNotCompleted;
  ...
  begin
    ...
    if Str[1] = 'O' then
      case Str[2] of
        'A':
          Result := ecAppCursorUp;
        'B':
          Result := ecAppCursorDown;
        'C':
          Result := ecAppCursorRight;
        'D':
          Result := ecAppCursorLeft;
      end;
  end;
end;
...
var
  ...
  begin
    ...
    begin
      ...
      case Str[1] of
        '5':
          Result := ecQueryDevice;
        '0':
          Result := ecReportDeviceOK;
        '3':
          Result := ecReportDeviceFailure;
        '6':
          Result := ecQueryCursorPos;
      else
        Result := ecUnknown;
      end;
    end;
...
{$IFDEF Unicode} if not CharInSet(LastCh, ['A' .. 'Z', 'a' .. 'z']) then
    Exit;
{$ELSE} if not(LastCh in ['A' .. 'Z', 'a' .. 'z']) then
    Exit; {$ENDIF}
  ...
  try
    ...
    case LastCh of
      ...
      'f':
        Result := ecCursorMove;
      'J':
        Result := CodeEraseScreen;
      'K':
        Result := CodeEraseLine;
      'g':
        Result := CodeTab;
      'm':
        Result := ecAttributes;
      ...
      's':
        Result := ecSaveCaret;
      'u':
        Result := ecRestoreCaret;
      'n':
        Result := CodeDevice;
      'c':
        Result := CodeIdentify;
      'R':
        Result := ecReportCursorPos;
    else
      Result := ecUnknown;
    end;
