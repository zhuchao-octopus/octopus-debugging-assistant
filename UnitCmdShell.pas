...
begin
  ...
  try
    ...
    StrPCopy(CmdLine, s);
    if CreateProcess(nil, CmdLine, nil, nil, true, REALTIME_PRIORITY_CLASS, nil,
      nil, SI, PI) then
    begin
      ExitCode := 0;
      ...
      begin
        ...
        if PeekNamedPipe(hReadPipe, @CommadLineDestBuffer[0], BufferSize,
          @AvailBytes, nil, nil) then
        begin
          if AvailBytes > 0 then
          begin
            try
              FillChar(CommadLineDestBuffer, Sizeof(CommadLineDestBuffer), #0);
              ReadFile(hReadPipe, CommadLineDestBuffer[0], AvailBytes,
                BytesRead, nil);
