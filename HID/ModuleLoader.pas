...
function ReadModuleData(Module: TModuleHandle; SymbolName: string; var Buffer;
  Size: Cardinal): Boolean;
function WriteModuleData(Module: TModuleHandle; SymbolName: string; var Buffer;
  Size: Cardinal): Boolean;
...
type
  TModuleLoadMethod = (ltDontResolveDllReferences, ltLoadAsDataFile,
    ltAlteredSearchPath);
  ...
  public
    ...
    class function IsAvaliable(const ADLLName: string;
      const AProcName: string = ''): Boolean;
    constructor Create(const ADLLName: string;
      LoadMethods: TModuleLoadMethods = []);
    ...
    function GetExportedSymbol(const AName: string; var Buffer;
    ...
    // Make sure AName is actually a symbol and not a function or this will crash horribly!
    function SetExportedSymbol(const AName: string; var Buffer;
      Size: Integer): Boolean;
...
constructor TModuleLoader.Create(const ADLLName: string;
  LoadMethods: TModuleLoadMethods = []);
begin
  inherited Create;
...
function TModuleLoader.GetProcedure(const AName: string;
  var AProc: Pointer): Boolean;
begin
  Result := Loaded;
...
class function TModuleLoader.IsAvaliable(const ADLLName: string;
  const AProcName: string = ''): Boolean;
var
...
{$ENDIF MSWINDOWS}
{$IFDEF UNIX}
  (RTLD_LAZY, RTLD_LAZY, RTLD_LAZY);
  // there is not really a equivalent under Linux
{$ENDIF UNIX}
var
  Flags: DWORD;
  I: TModuleLoadMethod;
