...
begin
  ...
  begin
    if HidDev.ProductName <> '' then
      ...
      N := DevListBox.Items.Add(Format('Device VID=%.4x PID=%.4x', [HidDev.Attributes.VendorID, HidDev.Attributes.ProductID]));
    HidCtl.CheckOutByIndex(Dev, Idx);
    DevListBox.Items.Objects[N] := Dev;
    HidDev.NumInputBuffers := SizeOf(TReport);
    HidDev.NumOverlappedBuffers := SizeOf(TReport);
  end;
  Result := True;
end;
...
begin
  ...
  ReadBtnClick(Self);
  HistoryListBox.Clear;
  if Assigned(Edits[0]) and (DevListBox.Items.Count > 0) and (DevListBox.ItemIndex >= 0) then
  begin
