...
type
  ...
  TJvHidEnumerateEvent = function(HidDev: TJvHidDevice; const Idx: Integer)
    : Boolean of object;
  ...
  TJvHidDataErrorEvent = procedure(HidDev: TJvHidDevice; Error: DWORD)
    of object;
  TJvHidDeviceCreateError = procedure(Controller: TJvHidDeviceController;
    PnPInfo: TJvHidPnPInfo; var Handled: Boolean; var RetryCreate: Boolean)
    of object;
...
{$ENDIF ~UNICODE}
  ...
  public
    ...
    function DeviceIoControl(IoControlCode: DWORD; InBuffer: Pointer;
      InSize: DWORD; OutBuffer: Pointer; OutSize: DWORD;
      var BytesReturned: DWORD): Boolean;
    function FlushQueue: Boolean;
    function GetButtonCaps(ButtonCaps: PHIDPButtonCaps; var Count: Word)
      : NTSTATUS;
    function GetButtons(UsageList: PUsage; var UsageLength: ULONG; var Report;
    ...
    function GetData(DataList: PHIDPData; var DataLength: ULONG; var Report;
      ReportLength: ULONG): NTSTATUS;
    function GetFeature(var Report; const Size: Integer): Boolean;
    function GetScaledUsageValue(var UsageValue: Integer; var Report;
      ReportLength: ULONG): NTSTATUS;
    function GetSpecificButtonCaps(ButtonCaps: PHIDPButtonCaps; var Count: Word)
      : NTSTATUS;
    function GetSpecificValueCaps(ValueCaps: PHIDPValueCaps; var Count: Word)
      : NTSTATUS;
    function GetUsages(UsageList: PUsage; var UsageLength: ULONG; var Report;
      ReportLength: ULONG): NTSTATUS;
    function GetUsagesEx(UsageList: PUsageAndPage; var UsageLength: ULONG;
      var Report; ReportLength: ULONG): NTSTATUS;
    function GetUsageValue(var UsageValue: ULONG; var Report;
      ReportLength: ULONG): NTSTATUS;
    function GetUsageValueArray(UsageValue: PAnsiChar;
    ...
    function SetButtons(UsageList: PUsage; var UsageLength: ULONG; var Report;
      ReportLength: ULONG): NTSTATUS;
    function SetData(DataList: PHIDPData; var DataLength: ULONG; var Report;
      ReportLength: ULONG): NTSTATUS;
    function SetFeature(var Report; const Size: Integer): Boolean;
    function SetScaledUsageValue(UsageValue: Integer; var Report;
      ReportLength: ULONG): NTSTATUS;
    function SetUsages(UsageList: PUsage; var UsageLength: ULONG; var Report;
      ReportLength: ULONG): NTSTATUS;
    function SetUsageValue(UsageValue: ULONG; var Report; ReportLength: ULONG)
      : NTSTATUS;
    function SetUsageValueArray(UsageValue: PAnsiChar;
      UsageValueByteLength: Word; var Report; ReportLength: ULONG): NTSTATUS;
    function UnsetButtons(UsageList: PUsage; var UsageLength: ULONG; var Report;
      ReportLength: ULONG): NTSTATUS;
    function UnsetUsages(UsageList: PUsage; var UsageLength: ULONG; var Report;
      ReportLength: ULONG): NTSTATUS;
    ...
    function WriteFile(var Report; ToWrite: DWORD;
      var BytesWritten: DWORD): Boolean;
    ...
    function GetExtendedAttributes(ReportType: THIDPReportType; DataIndex: Word;
      Attributes: PHIDPExtendedAttributes; var LengthAttributes: ULONG)
      : NTSTATUS;
    ...
    property PhysicalDescriptor: TJvPhysicalDescriptor
      read GetPhysicalDescriptor;
...
{$ENDIF UNICODE}
    ...
    property Configuration: THIDDConfiguration read GetConfiguration
      write SetConfiguration;
    property LinkCollectionParam: Word read FLinkCollectionParam
      write FLinkCollectionParam;
    property NumInputBuffers: Integer read FNumInputBuffers
      write SetNumInputBuffers;
    property NumOverlappedBuffers: Integer read FNumOverlappedBuffers
      write SetNumOverlappedBuffers;
    property ReportTypeParam: THIDPReportType read FReportTypeParam
      write SetReportTypeParam;
    property Tag: Integer read FTag write FTag;
    property PollingDelayTime: Integer read FPollingDelayTime
      write SetPollingDelayTime;
    property ThreadSleepTime: Integer read FThreadSleepTime
      write SetThreadSleepTime;
    property UsagePageParam: TUsage read FUsagePageParam
      write SetUsagePageParam;
...
{$ELSE}
    property DeviceStringsUnicode[Idx: Byte]: WideString
      read GetDeviceStringWideString;
{$ENDIF UNICODE}
    property LinkCollectionNodes[Idx: Word]: THIDPLinkCollectionNode
      read GetLinkCollectionNode;
    // event properties
    property OnData: TJvHidDataEvent read FOnData write SetOnData;
    property OnDataError: TJvHidDataErrorEvent read FOnDataError
      write FOnDataError;
...
{$ENDIF RTL230_UP}
  ...
  private
    ...
    function CheckThisOut(var HidDev: TJvHidDevice; Idx: Integer;
  ...
  public
    ...
    constructor Create(AOwner: TComponent;
      AOnHidCtlDeviceCreateError: TJvHidDeviceCreateError;
    ...
    function CheckOutByClass(var HidDev: TJvHidDevice;
      const ClassName: string): Boolean;
    function CheckOutByID(var HidDev: TJvHidDevice;
...
{$IFDEF UNICODE}string{$ELSE}WideString{$ENDIF}): Integer;
  ...
  published
    property DevPollingDelayTime: Integer read FDevPollingDelayTime
      write SetDevPollingDelayTime default 0;
    property DevThreadSleepTime: Integer read FDevThreadSleepTime
      write SetDevThreadSleepTime default 100;
    ...
    // the iterator event
    property OnEnumerate: TJvHidEnumerateEvent read FOnEnumerate
      write FOnEnumerate;
    // the central event for HID device changes
    property OnDeviceChange: TNotifyEvent read FOnDeviceChange
      write SetOnDeviceChange;
    // this event is triggered when an error occured while creating a given TJvHidDevice
    property OnDeviceCreateError: TJvHidDeviceCreateError
      read FOnDeviceCreateError write FOnDeviceCreateError;
    // these events are copied to TJvHidDevices on creation
    property OnDeviceData: TJvHidDataEvent read FOnDeviceData
      write SetOnDeviceData;
    property OnDeviceDataError: TJvHidDataErrorEvent read FOnDeviceDataError
      write SetOnDeviceDataError;
    property OnDeviceUnplug: TJvHidUnplugEvent read FOnDeviceUnplug
      write SetOnDeviceUnplug;
...
const
  UnitVersioning: TUnitVersionInfo = (RCSfile: '$URL$'; Revision: '$Revision$';
    Date: '$Date$'; LogPath: 'JVCL\run');
{$ENDIF UNITVERSIONING}
...
procedure DummyReadCompletion(ErrorCode: DWORD; Count: DWORD;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        SleepEx(Device.ThreadSleepTime, True);
        // avoid 100% CPU usage (Mantis 5749)
      end;
    end;
  finally
...
constructor TJvHidPnPInfo.Create(APnPHandle: HDEVINFO; ADevData: TSPDevInfoData;
  const ADevicePath: string);
begin
  inherited Create;
  ...
  // primary information
  FCapabilities := GetRegistryPropertyDWord(APnPHandle, ADevData,
    SPDRP_CAPABILITIES);
  FClassDescr := GetRegistryPropertyString(APnPHandle, ADevData, SPDRP_CLASS);
  FClassGUID := GetRegistryPropertyString(APnPHandle, ADevData,
    SPDRP_CLASSGUID);
  FCompatibleIDs := GetRegistryPropertyStringList(APnPHandle, ADevData,
    SPDRP_COMPATIBLEIDS);
  FConfigFlags := GetRegistryPropertyDWord(APnPHandle, ADevData,
    SPDRP_CONFIGFLAGS);
  FDeviceDescr := GetRegistryPropertyString(APnPHandle, ADevData,
    SPDRP_DEVICEDESC);
  FDriver := GetRegistryPropertyString(APnPHandle, ADevData, SPDRP_DRIVER);
  FFriendlyName := GetRegistryPropertyString(APnPHandle, ADevData,
    SPDRP_FRIENDLYNAME);
  FHardwareID := GetRegistryPropertyStringList(APnPHandle, ADevData,
    SPDRP_HARDWAREID);
  FLowerFilters := GetRegistryPropertyStringList(APnPHandle, ADevData,
    SPDRP_LOWERFILTERS);
  FMfg := GetRegistryPropertyString(APnPHandle, ADevData, SPDRP_MFG);
  FUpperFilters := GetRegistryPropertyStringList(APnPHandle, ADevData,
    SPDRP_UPPERFILTERS);
  ...
  FBusType := GetRegistryPropertyString(APnPHandle, ADevData,
    SPDRP_BUSTYPEGUID);
  FCharacteristics := GetRegistryPropertyString(APnPHandle, ADevData,
    SPDRP_CHARACTERISTICS);
  FDevType := GetRegistryPropertyDWord(APnPHandle, ADevData, SPDRP_DEVTYPE);
  FEnumeratorName := GetRegistryPropertyString(APnPHandle, ADevData,
    SPDRP_ENUMERATOR_NAME);
  FExclusive := GetRegistryPropertyDWord(APnPHandle, ADevData, SPDRP_EXCLUSIVE);
  FLegacyBusType := GetRegistryPropertyDWord(APnPHandle, ADevData,
    SPDRP_LEGACYBUSTYPE);
  FLocationInfo := GetRegistryPropertyString(APnPHandle, ADevData,
    SPDRP_LOCATION_INFORMATION);
  FPhysDevObjName := GetRegistryPropertyString(APnPHandle, ADevData,
    SPDRP_PHYSICAL_DEVICE_OBJECT_NAME);
  FSecuritySDS := GetRegistryPropertyString(APnPHandle, ADevData,
    SPDRP_SECURITY_SDS);
  FUINumber := GetRegistryPropertyDWord(APnPHandle, ADevData, SPDRP_UI_NUMBER);
  FUINumberFormat := GetRegistryPropertyString(APnPHandle, ADevData,
...
begin
  ...
  RegDataType := 0;
  Result := '';
  SetupDiGetDeviceRegistryProperty(PnPHandle, DevData, Prop, RegDataType, nil,
    0, BytesReturned);
  ...
  begin
    ...
    try
      ...
      SetupDiGetDeviceRegistryProperty(PnPHandle, DevData, Prop, RegDataType,
        PByte(@Buffer[0]), BytesReturned, BytesReturned);
      Result := Buffer;
    finally
...
constructor TJvHidDevice.CtlCreate(const APnPInfo: TJvHidPnPInfo;
  const Controller: TJvHidDeviceController);
begin
  inherited Create;
  ...
  FHidFileHandle := CreateFile(PChar(PnPInfo.DevicePath), GENERIC_READ or
...
begin
  ...
  if IsAccessible then
    ...
    begin
      ...
      begin
        FHidOverlappedRead := CreateFile(PChar(PnPInfo.DevicePath),
    ...
    begin
      if HidOverlappedWrite = INVALID_HANDLE_VALUE then // if not already opened
        FHidOverlappedWrite := CreateFile(PChar(PnPInfo.DevicePath),
          GENERIC_WRITE, FILE_SHARE_READ or FILE_SHARE_WRITE, nil,
...
begin
  if IsAccessible then
  begin
    FMaxDataListLength := HidP_MaxDataListLength(ReportTypeParam,
      PreparsedData);
    FMaxUsageListLength := HidP_MaxUsageListLength(ReportTypeParam,
      UsagePageParam, PreparsedData);
    FMaxButtonListLength := HidP_MaxButtonListLength(ReportTypeParam,
      UsagePageParam, PreparsedData);
  end;
end;
...
function TJvHidDevice.GetVendorName:
...
function TJvHidDevice.GetProductName:
{$IFDEF UNICODE}string{$ELSE}WideString{$ENDIF};
var
  Buffer: array [0 .. 253] of WideChar;
begin
...
function TJvHidDevice.GetSerialNumber:
{$IFDEF UNICODE}string{$ELSE}WideString{$ENDIF};
var
  I: Integer;
  Len: Integer;
...
begin
  if Length(FPhysicalDescriptor) = 0 then
    ...
    begin
      ...
      while not HidD_GetPhysicalDescriptor(HidFileHandle,
        FPhysicalDescriptor[0], I * SizeOf(Word)) do
      begin
        Inc(I);
...
begin
  if FLanguageStrings.Count = 0 then
    ...
    begin
      ...
      FillChar(IDs, SizeOf(IDs), $FF);
      Len := 0;
      if HidD_GetIndexedString(HidFileHandle, 0, PWideChar(@IDs), SizeOf(IDs))
      then
        for I := High(IDs) downto 0 do
          if IDs[I] <> $FFFF then
          begin
      ...
      begin
        Name[0] := #0;
        if GetLocaleInfo(Word(IDs[I]), LOCALE_SLANGUAGE, Name, SizeOf(Name)) <> 0
        then
...
begin
  ...
  if HidOverlappedWrite <> INVALID_HANDLE_VALUE then
    if not GetOverlappedResult(HidOverlappedWrite, FOvlWrite, Result, False)
    then
      Result := 0;
end;
...
begin
  ...
  else if (Mode = omhWrite) and (HidOverlappedWrite <> INVALID_HANDLE_VALUE)
  then
...
function TJvHidDevice.GetSpecificButtonCaps(ButtonCaps: PHIDPButtonCaps;
  var Count: Word): NTSTATUS;
begin
...
function TJvHidDevice.GetButtonCaps(ButtonCaps: PHIDPButtonCaps;
  var Count: Word): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_GetButtonCaps_(ReportTypeParam, ButtonCaps, Count,
      PreparsedData);
end;

function TJvHidDevice.GetSpecificValueCaps(ValueCaps: PHIDPValueCaps;
  var Count: Word): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function TJvHidDevice.GetValueCaps(ValueCaps: PHIDPValueCaps; var Count: Word)
  : NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_GetValueCaps_(ReportTypeParam, ValueCaps, Count,
      PreparsedData);
end;
...
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_GetUsages(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageList, UsageLength, PreparsedData, Report,
      ReportLength);
end;
...
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_GetButtons(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageList, UsageLength, PreparsedData, Report,
      ReportLength);
end;

function TJvHidDevice.GetUsagesEx(UsageList: PUsageAndPage;
  var UsageLength: ULONG; var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
function TJvHidDevice.GetButtonsEx(UsageList: PUsageAndPage;
  var UsageLength: ULONG; var Report; ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
...
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_SetUsages(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageList, UsageLength, PreparsedData, Report,
      ReportLength);
end;
...
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_SetButtons(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageList, UsageLength, PreparsedData, Report,
      ReportLength);
end;
...
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_UnsetUsages(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageList, UsageLength, PreparsedData, Report,
      ReportLength);
end;
...
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_UnsetButtons(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageList, UsageLength, PreparsedData, Report,
      ReportLength);
end;

function TJvHidDevice.GetUsageValue(var UsageValue: ULONG; var Report;
  ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_GetUsageValue(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageParam, UsageValue, PreparsedData, Report,
      ReportLength);
end;

function TJvHidDevice.GetScaledUsageValue(var UsageValue: Integer; var Report;
  ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_GetScaledUsageValue(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageParam, UsageValue, PreparsedData, Report,
      ReportLength);
end;
...
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_GetUsageValueArray(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageParam, UsageValue, UsageValueByteLength,
      PreparsedData, Report, ReportLength);
end;

function TJvHidDevice.SetUsageValue(UsageValue: ULONG; var Report;
  ReportLength: ULONG): NTSTATUS;
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_SetUsageValue(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageParam, UsageValue, PreparsedData, Report,
      ReportLength);
end;
...
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_SetScaledUsageValue(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageParam, UsageValue, PreparsedData, Report,
      ReportLength);
end;
...
begin
  Result := HIDP_STATUS_NULL; // for not plugged in
  if IsAccessible then
    Result := HidP_SetUsageValueArray(ReportTypeParam, UsagePageParam,
      LinkCollectionParam, UsageParam, UsageValue, UsageValueByteLength,
      PreparsedData, Report, ReportLength);
end;

function TJvHidDevice.DeviceIoControl(IoControlCode: DWORD; InBuffer: Pointer;
  InSize: DWORD; OutBuffer: Pointer; OutSize: DWORD;
  var BytesReturned: DWORD): Boolean;
begin
  Result := False;
  if OpenFile then
    Result := Windows.DeviceIoControl(HidFileHandle, IoControlCode, InBuffer,
      InSize, OutBuffer, OutSize, BytesReturned, nil);
end;
...
function TJvHidDevice.WriteFile(var Report; ToWrite: DWORD;
  var BytesWritten: DWORD): Boolean;
begin
  Result := False;
  if OpenFile then
    Result := Windows.WriteFile(HidFileHandle, Report, ToWrite,
      BytesWritten, nil);
end;
...
begin
  ...
  begin
    ...
    Result := JvHidControllerClass.ReadFileEx(HidOverlappedRead, Report, ToRead,
      FOvlRead, CallBack);
  end;
end;
...
begin
  ...
  begin
    ...
    Result := JvHidControllerClass.WriteFileEx(HidOverlappedWrite, Report,
      ToWrite, FOvlWrite, CallBack);
  end;
end;
...
function TJvHidDevice.GetExtendedAttributes(ReportType: THIDPReportType;
  DataIndex: Word; Attributes: PHIDPExtendedAttributes;
  var LengthAttributes: ULONG): NTSTATUS;
begin
...
function TJvHidDevice.InitializeReportForID(ReportType: THIDPReportType;
  ReportID: Byte; var Report; ReportLength: ULONG): NTSTATUS;
begin
  if Assigned(HidP_InitializeReportForID) then
    Result := HidP_InitializeReportForID(ReportType, ReportID, FPreparsedData,
      Report, ReportLength)
  else
    Result := HIDP_STATUS_NOT_IMPLEMENTED;
end;
...
begin
  ...
  Create(AOwner, TJvHidDeviceCreateError(Method));
  // work around compiler issue with method overloading and method pointers
end;

constructor TJvHidDeviceController.Create(AOwner: TComponent;
  AOnHidCtlDeviceCreateError: TJvHidDeviceCreateError;
  AOnDeviceChange: TNotifyEvent);
const
...
begin
  ...
  if not(csDestroying in ComponentState) and (Msg.Msg = WM_DEVICECHANGE) and
    (TWMDeviceChange(Msg).Event = DBT_DEVNODES_CHANGED) then
...
var
  ...
  begin
    ...
    PnPHandle := SetupDiGetClassDevs(@FHidGuid, nil, 0, DIGCF_PRESENT or
    ...
    repeat
      ...
      Success := SetupDiEnumDeviceInterfaces(PnPHandle, nil, FHidGuid, Devn,
        DeviceInterfaceData);
      if Success then
      begin
        ...
        SetupDiGetDeviceInterfaceDetail(PnPHandle, @DeviceInterfaceData, nil, 0,
          BytesReturned, @DevData);
...
begin
  ...
  begin
    ...
    for J := NewList.Count - 1 downto 0 do
      if (TJvHidDevice(NewList.Items[J]).PnPInfo.DeviceID = HidDev.PnPInfo.
        DeviceID) and HidDev.IsPluggedIn then
      begin
        HidDev := nil;
        Break;
      end;
  ...
  for I := 0 to NewList.Count - 1 do
    for J := 0 to FList.Count - 1 do
      if (TJvHidDevice(NewList[I]).PnPInfo.DeviceID = TJvHidDevice(FList[J])
        .PnPInfo.DeviceID) and TJvHidDevice(FList[J]).IsPluggedIn then
      begin
        TJvHidDevice(NewList[I]).FMyController := nil;
        // prevent Free/Destroy from accessing this controller
...
begin
  ...
  begin
    SetLength(Buf, Size);
    GetFileVersionInfo(HidModuleName, DWORD(INVALID_HANDLE_VALUE),
      Size, @Buf[0]);
    if VerQueryValue(@Buf[0], 'StringFileInfo\040904E4\FileVersion',
      Pointer(Value), Size) then
      Result := Value;
  end;
end;
...
procedure TJvHidDeviceController.SetOnDeviceChange(const Notifier
  : TNotifyEvent);
begin
...
function TJvHidDeviceController.DoEnumerate(HidDev: TJvHidDevice;
  Idx: Integer): Boolean;
begin
  Result := False;
...
procedure TJvHidDeviceController.SetOnDeviceData(const DataEvent
  : TJvHidDataEvent);
var
  I: Integer;
  Dev: TJvHidDevice;
...
procedure TJvHidDeviceController.SetOnDeviceDataError(const DataErrorEvent
  : TJvHidDataErrorEvent);
var
  I: Integer;
  Dev: TJvHidDevice;
begin
...
procedure TJvHidDeviceController.SetOnDeviceUnplug(const Unplugger
  : TJvHidUnplugEvent);
var
  I: Integer;
  Dev: TJvHidDevice;
...
function TJvHidDeviceController.CheckThisOut(var HidDev: TJvHidDevice;
  Idx: Integer; Check: Boolean): Boolean;
begin
...
begin
  ...
  if ProductName <> '' then
    for I := 0 to FList.Count - 1 do
    begin
      Result := CheckThisOut(HidDev, I, ProductName = TJvHidDevice(FList[I])
        .ProductName);
      if Result then
        Break;
    end;
...
begin
  ...
  if VendorName <> '' then
    for I := 0 to FList.Count - 1 do
    begin
      Result := CheckThisOut(HidDev, I, VendorName = TJvHidDevice(FList[I])
        .VendorName);
      if Result then
        Break;
    end;
...
begin
  ...
  if ClassName <> '' then
    for I := 0 to FList.Count - 1 do
    begin
      Result := CheckThisOut(HidDev, I, ClassName = TJvHidDevice(FList[I])
        .PnPInfo.ClassDescr);
      if Result then
        Break;
    end;
...
function TJvHidDeviceController.CountByCallback
  (Check: TJvHidCheckCallback): Integer;
var
  I: Integer;
  Dev: TJvHidDevice;
begin
